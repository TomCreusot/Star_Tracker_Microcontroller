...
use crate::image_processing::RefWordImage;
use crate::image_processing::Image;
impl <'a, const WIDTH : usize, const HEIGHT : usize> RefWordImage <'a, WIDTH, HEIGHT>
{
//###############################################################################################//
//										---	Constructors ---
//###############################################################################################//
	...
	/// Borrows the image.
	pub fn new ( img: &'a mut [[Word; WIDTH]; HEIGHT] ) -> Self	{ Self { img: img } }
}


impl <'a, const WIDTH : usize, const HEIGHT : usize> Image for RefWordImage <'a, WIDTH, HEIGHT>
{
//###############################################################################################//
//										---	Accessors ---
//###############################################################################################//
	...
	{
		let val = self.img[pixel.y][pixel.x / self.word_size.nibble_size];
		return val.get(pixel.x % self.word_size.nibble_size, self.word_size);
	}
	...
	{
		let val = self.img[pixel.y][pixel.x / self.word_size.nibble_size];
		val.set(pixel.x % self.word_size.nibble_size, self.word_size);
		self.img[pixel.y][pixel.x / self.word_size.nibble_size] = val;
	}
