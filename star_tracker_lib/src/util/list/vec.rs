//! Addition to [Linear Lookup](crate::util::list) for Vec.
...
//										~ pop ~													 //
	#[test]
	// A vector should be able to pop an element if it is not empty.
	fn test_pop_vec_not_empty ( )
	{
	...
	#[test]
	// If the vector is empty, it should output an error if a pop is attempted.
	fn test_pop_vec_empty ( )
	{
		let mut list: Vec<usize> = Vec::new();
		assert_eq!((&mut list as &mut dyn List<usize>).pop(0), Err(Errors::InvalidSize));
	}
	
	#[test]
	// If the index is out of bounds, the vector should produce an error.
	fn test_pop_vec_out_of_bounds ( )
	{
		let mut list: Vec<usize> = vec!(1,2,3);
		assert_eq!((&mut list as &mut dyn List<usize>).pop(4), Err(Errors::OutOfBounds));
	}
