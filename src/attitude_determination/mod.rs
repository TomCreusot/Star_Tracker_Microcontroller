...
//! *-* Incredibly confusing and lack of instructions.
//!
...
//! *-* Potential singularity (CRP coordinates).
//! *-* Incredibly confusing and lack of instructions.
//!
...
// 	use attitude_determination::Davenport;
// 	use attitude_determination::Triad;
	...
	{
		const LAMBDA_PRECISION : Decimal = 0.0000001;
	}
		...
		) -> ArrayList<Match<Cartesian3D>, N>
	{
		...
		while !coords.is_full()
		{
			...
			axis.normalize();
			let output = Quaternion::new(AngleAxis{angle: angle, axis: axis}).rotate_point(input);
	...
	fn test_quest_matlab ( )
	{
		...
		input.push_back(Match{input: Cartesian3D{x: 1.0, y: 0.0, z: 0.3}, output: Cartesian3D{x: 0.1, y: 1.0, z: 1.0}, weight: 1.0}).expect("HUH");
		input.push_back(Match{input: Cartesian3D{x: 1.0, y: 0.4, z: 0.0}, output: Cartesian3D{x: 0.5, y: 1.0, z: 1.0}, weight: 1.0}).expect("HUH");
		input.push_back(Match{input: Cartesian3D{x: 1.0, y: 0.3, z: 0.0}, output: Cartesian3D{x: 0.3, y: 1.0, z: 1.0}, weight: 1.0}).expect("HUH");
		input.push_back(Match{input: Cartesian3D{x: 1.0, y: 0.5, z: 0.0}, output: Cartesian3D{x: 0.2, y: 1.0, z: 1.0}, weight: 1.0}).expect("HUH");
		...
		let mut output_aa = AngleAxis::new(&output_q);
		let mut expected_aa = AngleAxis{
			angle: Radians(1.982725766902095), 
			axis: Cartesian3D{x: -0.550977167382162, y: -0.823663924691362, z:-0.134171160036543}};
		
		output_aa.axis.normalize();
		expected_aa.axis.normalize();
		
		assert_eq!(Degrees(0.0), output_aa.compare(expected_aa).to_degrees());
	}
	...
	fn test_quest_perfect_values ( )
	{
		...
		let ang_out = AngleAxis::new(&rotation);
		println!("angle out: {:?}", ang_out);
		
		assert_eq!(Radians(0.0), ang_out.compare(angle_axis));
	}
	...
	fn test_quest_bad_axis ( )
	{
		...
		println!("{}", rotation.dot(Quaternion::new(angle_axis)).abs());
		assert!(rotation.dot(Quaternion::new(angle_axis)).abs() < 0.01);
		// assert_eq!(Radians(0.0), ang_out.compare(angle_axis));
	}
