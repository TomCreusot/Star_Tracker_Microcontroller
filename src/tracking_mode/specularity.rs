...
impl Specularity
{
	...
	/// ```
	pub fn new <T> ( triangle: &StarTriangle<Cartesian3D> ) -> Specularity
...
mod test
{
	...
	//
	// fn same ( &Cartesian3D, &Cartesian3D ) -> bool
	//
	#[test]
	fn test_same_ignore ( )
	{
		...
		let st2 : StarTriangle<Cartesian3D> = StarTriangle(pt1.clone(), pt3.clone(), pt2.clone() );
		// Ignore
		let st3 : StarTriangle<Cartesian3D> = StarTriangle(pt1.clone(), pt1.clone(), pt1.clone() );
