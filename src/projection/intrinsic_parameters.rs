...
{	
	pub fn to_image ( &self, point: Cartesian3D ) -> PixelWeighted
	{
		...
		return PixelWeighted{x: out_2d.x, y: out_2d.y};
	}
	
	
	// pub fn from_image ( &self, point: PixelWeighted ) -> Cartesian3D
	// {
	// 	let x = (point - self.principle_point.x) / self.focal_length;
	// 	let y = (point - self.principle_point.y) / self.focal_length;
	// 	return Cartesian3D{x: x, y: y, z}
	// }
	...
	pub fn from_fov ( fov: Radians, img_size: Decimal ) -> Self
	{
		...
		let focal_lengths = PixelWeighted{x: focal_length, y: focal_length};
		let principle_point = PixelWeighted{x: img_size / 2.0, y: img_size / 2.0};
