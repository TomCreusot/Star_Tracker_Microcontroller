...
{
	println!("READING");
	let mut rgb : RgbImage = NixImage::read_image("sample_in.png");
	let mut img : BasicImage<2000, 2000> = BasicImage::new();
	println!("Converting To Image");
	NixImage::dynamic_to_image(&rgb, &mut img);
	...
	for i in 0..stars.size()
	{
		...
		NixImage::draw_points(x, y, size, color, &mut rgb );
	}
